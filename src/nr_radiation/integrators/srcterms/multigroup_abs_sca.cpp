...
Real RadIntegrator::MultiGroupAbsScat(
  ...
  AthenaArray<Real> &implicit_coef, AthenaArray<Real> &ir_cm, Real &tem_diff) {
...
Real kappa_pm = 1.0, kappa_rm = 1.0;
...
while ((count_iteration <= iteration_tgas_) && (tgas_diff > tgas_error_)) {
  ...
  for (int ifr=0; ifr<nfreq; ++ifr) {
    ...
    cal_opacity(rho, tgasnew, ifr, kappa_pm, kappa_rm);

    sigma_a[ifr] = rho*kappa_rm;
    sigma_s[ifr] = 0.0;
    sigma_pe[ifr] = rho*kappa_pm;
    sigma_p[ifr] = rho*kappa_pm;
  ...
  count_iteration++;
  tgas_last = tgasnew;
  // std::cout << "count_iteration: " << count_iteration << std::endl;
  // std::cout << "tgas_diff: " << tgas_diff << std::endl;
}
